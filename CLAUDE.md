...
Homi는 가족용 퍼스널 자동화 프로젝트입니다. 다섯 가지 주요 서브시스템으로 구성됩니다:
...
5. **떨사오팔** (`ddsaopal/`) — TypeScript, 토스증권 API, "떨어지면 사고 오르면 판다" n분할 매수 자동매매(전일종가 LOC 매수 / +0.3% LOC 매도 / 12일 저가LOC 손절), 집 맥 launchd
...
| 떨사오팔(n분할 매수) | `ddsaopal/src/main-{open,close}.ts`, `calculator.ts`, `tossApi.ts` | `ddsaopal/SPEC.md`, `ddsaopal/docs/RUNBOOK.md` |
...
| `ddsaopal/` | TypeScript | 떨사오팔 — 토스 API n분할 매수 자동매매(전일종가 LOC 매수/+0.3% LOC 매도/12일 손절). 코어 순수함수(`calculator.ts`)+토스클라(`tossApi.ts`), infinite-buy와 완전 분리 | `src/main-{open,close}.ts`, `src/{calculator,tossApi,state}.ts`, `SPEC.md` |
...
| `docs/02-architecture.md` | 현행 아키텍처 서술(5 서브시스템 + VR + 외부 API) | ✅ 최신(2026-07-23 현행화). CLAUDE.md와 충돌 시 CLAUDE.md 우선 |
...
| `ddsaopal/SPEC.md` (+ `README.md`, `docs/RUNBOOK.md`) | **떨사오팔 권위 사양** (n분할/전일종가 LOC 매수/+0.3% 매도/12일 손절/사이클, 토스 MOC 미지원→저가 LOC). RUNBOOK=운영/장애대응 | ✅ 최신, 떨사오팔 진실원 |
...
사용처: `infinite-buy/`, `signal-alert/`, `lotto/`, `scripts/vr-price-alert.js`, `ddsaopal/` 전부. Alram🔔은 **발신 전용** — 사용자가 답장해도 읽는 프로그램 없음.

> 과거에 로또용(→이후 Claude Code 텔레그램 플러그인 채널용)으로 쓰던 **Homi** (`@homi1801_bot`, 토큰 시작 `8317143838`) 봇은 **완전 미사용** (2026-07-22 텔레그램 플러그인·채널 설정 정리로 마지막 용도도 제거, 코드/설정 참조 없음). 봇 자체는 텔레그램에 살아있지만 아무데서도 쓰지 않음.
...
| **떨사오팔** 장 오픈/마감 | macOS launchd (집 맥). open KST 04:00·close KST 07:00(화~토), `open.sh`/`close.sh`→`npx tsx`, `ddsaopal-bot` 신원 자체 커밋. DRY-RUN 기본(`config.enabled`+`DDSAOPAL_LIVE_ORDERS=YES_REALLY` 둘 다 있어야 실주문) | `ddsaopal/launchd/com.homi.ddsaopal-{open,close}.plist`, `ddsaopal/{open,close}.sh` |
...
| VR 가격 알림 (장중 매시 매수/매도점 도달 감시 → Alram🔔) | macOS launchd (매시 05분 기동, 토스 캘린더로 KR/US 개장 판정 후 장중에만 조회) | `com.homi.vr-price-alert.plist` → `scripts/vr-price-alert.js` |
...
| Gmail 안읽음 정리 (기계성 보관처리 + 중요건 요약 알림) | 09:10 | `financial/gmail_triage.py` | `com.homi.gmail-triage.plist` | ✅ 운영중 |
...
- **Gmail 안읽음 정리**: 최근 2일(`LOOKBACK_DAYS`) 안읽음 받은편지함 수집 → 간단 기계필터 최소화, **Claude CLI(sonnet, 도구차단 `--allowedTools ""`)로 중요도 분류** → 기계성(로그인·프로모션·정기통지·명세서 등)은 **보관처리(UNREAD+INBOX 제거: 읽음+아카이브)**, 중요건은 **받은편지함에 안읽음으로 남기고 Alram🔔로 요약 전송**. 결과적으로 받은편지함엔 읽어야 할 메일만 남음. 스코프는 `gmail.modify`, 삼성카드와 **토큰 분리**(`financial/token_modify.json`, credentials.json만 공유). 최초 1회 동의: `python gmail_triage.py --auth`. 밀린 backlog 전체 1회 정리는 `--all`, 안전점검은 `--dry-run`.
