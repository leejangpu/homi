...
| 삼성카드 동기화 | `financial/sync_samsungcard.py`(+`fetch_`/`decrypt_samsungcard.py`) | [[project_samsungcard_decrypt]] (메모리) |
...
| 시스템 헬퍼(LAN IP·VR 리마인더·자동커밋) | `scripts/{lan-ip-notify.sh,vr-reminder.js,auto-commit.sh}` | (코드가 진실원) |
