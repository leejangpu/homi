...
- **가계부 웹**: 로컬 Express 서버 (`financial/server.js`, port 3000). LAN 내 접속 전용(외부 터널 없음). `financial/index.html`이 CSV/JSON을 fetch하여 차트/테이블 렌더링. 저장·영수증 분석·VR 상태 업데이트 모두 same-origin (`/save`, `/analyze-receipt`, `/save-vr` 등)으로 server.js가 직접 처리
