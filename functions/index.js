...
function toTelegramChunks(text) {
  ...
  if (content.length <= TELEGRAM_CONTENT_LIMIT) {
    return [content];
  }
...
function getPrimaryPhoto(photos) {
  ...
  return photos.reduce((best, current) => {
    ...
    const bestSize =
      typeof best.file_size === "number" && best.file_size > 0
        ? best.file_size
        : (best.width || 0) * (best.height || 0);
    const currentSize =
      typeof current.file_size === "number" && current.file_size > 0
        ? current.file_size
...
function buildImageInputs(message, maxImageCount) {
  ...
  if (photo?.file_id) {
    images.push({
      ...
      declaredSize: typeof photo.file_size === "number" ? photo.file_size : null,
      mimeType: "image/jpeg"
    });
  }
  ...
  if (document?.file_id && document?.mime_type?.startsWith("image/")) {
    images.push({
      ...
      declaredSize: typeof document.file_size === "number" ? document.file_size : null,
...
function buildPromptText(senderName, messageText, imageCount, skippedImageCount) {
  ...
  if (skippedImageCount > 0) {
    lines.push(`Skipped image files: ${skippedImageCount} (unsupported or too large)`);
  }
...
async function telegramApiRequest(botToken, method, body) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Telegram API failed (${method}, ${response.status}): ${errorText.slice(0, 400)}`
    );
  }
  ...
  if (!payload?.ok) {
    throw new Error(
      `Telegram API returned error (${method}): ${String(payload?.description || "unknown").slice(
        0,
        400
      )}`
    );
...
async function downloadTelegramImagePart(botToken, image, maxImageBytes) {
  if (image.declaredSize && image.declaredSize > maxImageBytes) {
    return null;
  }

  const file = await telegramApiRequest(botToken, "getFile", {
  ...
  if (typeof file.file_size === "number" && file.file_size > maxImageBytes) {
    return null;
  }
...
async function askGemini(
  ...
  const prompt = buildPromptText(
    ...
    messageText,
    imageParts.length,
    Math.max(imageInputs.length - imageParts.length, 0)
  );
  ...
  const response = await fetch(endpoint, {
    ...
    body: JSON.stringify({
      systemInstruction: {
        parts: [
          {
            text: "You are a helpful assistant for Telegram chat. Reply in Korean unless asked otherwise."
          }
        ]
      },
      contents: [
...
async function downloadTelegramFile(botToken, fileId) {
  ...
  if (typeof file.file_size === "number" && file.file_size > MAX_XLSX_BYTES) {
    throw new Error("파일이 너무 큽니다 (최대 10MB)");
  }

  const fileResponse = await fetch(
    `${TELEGRAM_API_BASE}/file/bot${botToken}/${file.file_path}`
  );
...
function formatLedgerSummary(transactions) {
  if (transactions.length === 0) return "파싱된 거래 내역이 없습니다.";

  const total = transactions.reduce((sum, tx) => sum + (Number(tx.amount) || 0), 0);
  const lines = [
    `카드 결제내역 ${transactions.length}건 파싱 완료`,
  ...
  if (transactions.length > 20) {
    lines.push(`... 외 ${transactions.length - 20}건`);
  }
...
exports.telegramWebhook = onRequest(
  ...
  async (req, res) => {
    ...
    // --- xlsx document handling (parse and reply with summary, no DB save) ---
    if (isXlsxDocument(message?.document)) {
      const geminiModel = (process.env.GEMINI_MODEL || DEFAULT_GEMINI_MODEL).trim();

      try {
        await telegramApiRequest(botToken, "sendChatAction", {
          chat_id: chatId,
          action: "typing"
        });
        ...
        const summary = formatLedgerSummary(transactions);
        await sendTelegramReply(botToken, message, summary);
      } catch (error) {
    ...
    // --- normal text/image handling ---
    const messageText = String(message?.text || message?.caption || "").trim();
    const maxImageCount = parsePositiveInt(
      process.env.TELEGRAM_MAX_IMAGE_COUNT,
      DEFAULT_MAX_IMAGE_COUNT
    );
    const maxImageBytes = parsePositiveInt(
      process.env.TELEGRAM_MAX_IMAGE_BYTES,
    ...
    try {
      ...
      try {
        await sendTelegramReply(
          botToken,
          message,
          "Request failed while processing your message. Please try again shortly."
        );
      } catch (replyError) {
...
exports.aiCommand = onCall({ region: "us-central1" }, async (request) => {
  if (!request.auth) {
  ...
  if (!text) {
    throw new HttpsError("invalid-argument", "text 값이 필요합니다.");
  }

  return {
    accepted: true,
    message: "AI command shell accepted. 실제 LLM 연동은 추후 구현 예정입니다.",
    echo: {
