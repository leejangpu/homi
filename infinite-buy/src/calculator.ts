...
export interface BuyOrder {
  ...
  price: number;
  quantity: number;
  amount: number;         // 실제 체결 예정 금액 = price × quantity (floor 후)
  label: string;
  intendedAmount?: number; // 이 단에 배정된 원 예산(반매수금/1회매수금). 큰수 클램프 시 수량 재계산 기준
}
...
export interface LargeNumberBuyResult {
  orders: BuyOrder[];   // 클램프가 반영된 매수 주문 (개수 불변)
  largeNum: number;     // 큰수 자리 가격 = 기준가 × (1 + pct/100)
  clampedCount: number; // 큰수로 눌린 주문 수
}

/**
 * 큰수 매수(가격 클램프) 후처리 — v4 언이시트 큰수 개념 중 "가격 클램프"만 v2/v3에 이식.
 *
 * 증권사는 기준가 대비 크게 벗어난 LOC 주문을 거부한다. 큰 하락 시 별지점/평단
 * 주문이 거부되어 "쌀 때 못 사는" 사고를 막기 위해, 각 매수 주문 가격을
 * 큰수(=기준가×(1+pct))로 클램프하고 **그 가격에서 수량을 재계산**한다
 * (의도 금액 유지 → 가격만 눌러 매수량이 줄던 기존 결함 교정).
 *
 * ⚠️ 무한매수법 원형대로 **주문 개수는 그대로**(전반 2건/후반 1건). 언이시트 v4의
 *    '대폭락 1주씩 티어'는 v2/v3에 넣지 않는다(v4 고유 요소, 사용자 결정 2026-07-20).
 *
 * 기준가(basePrice)는 실행부에서 전달(장중 실행 시 현재가 = 당일 종가 근사, LOC는 종가 체결).
 * calculate() 코어는 건드리지 않는 순수 후처리 함수.
 */
export function applyLargeNumberBuy(
  ...
  for (const bo of orders) {
    if (largeNum > 0 && bo.price > largeNum) {
      // 원 배정 예산(반매수금/1회매수금)으로 재계산 → 낮아진 가격에 맞춰 의도한 만큼 매수.
      // intendedAmount가 없으면(구주문 등) 기존 체결금액으로 폴백.
      const budget = bo.intendedAmount && bo.intendedAmount > 0 ? bo.intendedAmount : bo.amount;
  ...
  return { orders, largeNum, clampedCount };
}
