...
echo "▶ Claude 경로: $(which claude 2>/dev/null || echo 'claude not found')"
echo "▶ 파일 확인: $(ls -lh "$FILE_PATH" 2>&1)"
PROMPT_FILE=$(mktemp)
printf '%s' "$CLAUDE_PROMPT" > "$PROMPT_FILE"
RAW=$(claude -p --model claude-sonnet-4-6 --dangerously-skip-permissions --allowedTools "Read" --add-dir "$SCRIPT_DIR/tmp" < "$PROMPT_FILE" 2>&1)
rm -f "$PROMPT_FILE"
