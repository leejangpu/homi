...
- 금액 표기: 10,000원 배수는 반드시 한글 단위 사용 (예: 1,000,000원→100만원, 5,000,000원→500만원, 100,000,000원→1억원). 단, 10,000원 배수가 아닌 금액(예: 976,540원)은 숫자 그대로 표기
...
섹션 2: 은퇴 플랜 코칭 (반드시 포함, 구분선: <br><span class="highlight">── 은퇴 플랜 코칭 ──</span><br>)
전제: 2026~2036년 10년 내 순자산 15억 달성이 목표다.

이번 달 지출과 저축 데이터를 근거로 냉철하게 평가하고 구체적으로 조언해라.
- 현상 설명 금지. 평가와 조언만.
- 좋게 봐주지 마라. 목표 대비 부족하면 직접적으로 말해라.
- 구체적인 금액으로 말해라 (예: 이번 달 외식비 X만원은 Y만원 이하로 줄여야 한다).
- 지금 당장 해야 할 행동 2~3가지를 제시해라.
- 10년 목표 달성 가능성에 대한 현실적 판단을 내려라.
- 한국어 작성
