...
| `src/tossApi.ts` | (2단계) 토스 LOC 주문/보유/매수가능금액/캘린더 |
| `src/main-close.ts` / `src/main-open.ts` | (2단계) 마감 반영·계획 / 개장 제출 |

## 실행

```bash
cd ddsaopal && npm install
npm test            # 코어 로직 시나리오 검증
```

실주문은 3중 가드: `config.enabled=true` + `DDSAOPAL_LIVE_ORDERS=YES_REALLY` + (기본 DRY-RUN). 자세한 건 SPEC §11.
